...
func TestReceiptsCodeDec(t *testing.T) {
	...
	log1 := &Log{
		...
		Topics:      []crypto.HashType{*blockHash, *txHash},
		Data:        blockHash.Bytes(),
		BlockNumber: 123453,
		TxHash:      *txHash,
		TxIndex:     654321,
		BlockHash:   *blockHash,
	...
	rc := NewReceipt(txHash, contractAddr, true, tests[0].failed, tests[0].gasUsed, nil, []*Log{log1}).
	...
	for _, v := range tests {
		...
		contractAddr = contractAddress.Hash160()
		rc := NewReceipt(txHash, contractAddr, true, v.failed, v.gasUsed, nil, nil).WithTxIndex(v.ti)
		rcs.Append(rc)
	}
