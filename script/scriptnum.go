...
// Modified based on "github.com/btcsuite/btcd/txscript". Since scriptNum is not exported, we manually modify here.
...
var (
	scriptNumZero, _ = newScriptNum(operandFalse)
)
...
// A wrapper
func newScriptNum(op Operand) (scriptNum, error) {
	return makeScriptNum([]byte(op), false, defaultScriptNumLen)
}
